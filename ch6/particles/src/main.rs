...
unsafe impl GlobalAlloc for ReportingAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        eprintln!("alloc bytes: {},\tspend time: {}ns", layout.size(), (end - start).as_nanos());
        ptr
    }
...
impl Particle {
    fn new(world: &World) -> Particle {
        ...
        let x: f64 = rng.gen_range(0.0..=world.width);
        let y: f64 = world.height;
        let x_velocity = 0.0;
...
impl World {
    ...
    fn add_shapes(&mut self, n: i32) {
        for _ in 0..n.abs() {
            ...
            let box_particle = Box::new(particle);
            self.particles.push(box_particle);
        }
    }
